...
<head>
    <script>
        var myChart;
        $(function () {
            $.getJSON('/jiaju/res/js/basicTheme.json', function (themeJSON) {
        ...
        function FormatDate(strTime) {
         ...
         $(function () {
        	 $.getJSON('/jiaju/res/js/basicTheme.json', function (themeJSON) {
                 echarts.registerTheme('basicTheme', themeJSON);
                 // 基于准备好的dom，初始化eCharts实例
                 myChart = echarts.init($("#chart2Div")[0], "basicTheme");
                 var d1 = "";
                 var d2 = "";
                 var d3 = "";
                 var d4 = "";
                 var d5 = "";
                 $.ajax({
                	 url: "/jiaju/admin/home/piecharts",
                     type: "get",
                     dataType: "json",
                     async:false,
                     success: function (data) {
                    	//遍历json数组时，这么写dt为索引，0,1
              			for(var dt in data){
              				if(data[dt].pc == "卧室"){d1=data[dt].num}
              				if(data[dt].pc == "办公"){d2=data[dt].num}
              				if(data[dt].pc == "浴室"){d3=data[dt].num}
              				if(data[dt].pc == "客厅"){d4=data[dt].num}
              				if(data[dt].pc == "厨房"){d5=data[dt].num}
               			}
                     }
                 });
                 // 指定图表的配置项和数据
...
<body>
<ul id="chartByDate">
    <li><span class="chartDateBtn text_info">最近一周</span></li>
...
<div>
	<div id="chartDiv" style="width: 50%;height: 500px;float: left;"></div>
	<div id="chart2Div" style="width: 50%;height: 500px;float: left;"></div>
</div>
