      ...
      //单击取消发货按钮时
       function quxiao(id) {
    	   layer.msg('你确定取消么？', {
    		   ...
    		   ,yes: function(index){
    			   $.ajax({
    	               url: "admin/order_quxiao/" + id,
    	               type: "PUT",
    	               data: null,
    	               success: function (data) {
    	                   if (data.success) {
    	                	   layer.msg('已取消', {icon: 5});
    	                	   $('#btn_productOrder_refresh').trigger("click");
    	                   }
    	               },
    	               beforeSend: function () {
...
<div class="data_count_div text_info">
    ...
    <span class="data_count_title"></span>
    <span>订单总数:</span>
...
<div class="table_normal_div">
    ...
    <c:forEach items="${requestScope.productOrderList}" var="productOrder">
        <tr>
            ...
            <td><span title="取消订单"><a href="javascript:void(0)" onclick="quxiao(${productOrder.productOrder_id})" style="color: red;font-weight: normal !important;">取消订单</a></td>
        </tr>
    </c:forEach>
    </tbody>
    </table>
