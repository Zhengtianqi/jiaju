...
function getUserProfilePicture(username) {
    if(username !== null && username !== ""){
        $.getJSON("/jiaju/admin/login/profile_picture",{"username":username},function (data) {
            if(data.success){
                if(data.srcString !== null){
                    $("#img_profile_picture").attr("src", "/jiaju/res/images/item/adminProfilePicture/" + data.srcString);
                    return true;
                }
            }
        });
    }
    $("#img_profile_picture").attr("src","/jiaju/res/images/admin/loginPage/default_profile_picture-128x128.png");
}